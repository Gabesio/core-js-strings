...
function removeLastOccurrences(str, value) {
  ...
  return str.substring(0, index) + str.substring(index).replace(value, '');
}
...
function countVowels(str) {
  ...
  const res = str.split('').filter((a) => vowels.includes(a));
  return res.length;
}
...
function isPalindrome(str) {
  return (
    str.split('').reverse().join('').replace(/\s/g, '').toLowerCase() ===
...
function reverseWords(str) {
  ...
  const reversed = words.map((a) => a.split('').reverse().join(''));
